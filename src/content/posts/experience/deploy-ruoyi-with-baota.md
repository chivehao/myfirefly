...
有人可能会想：既然跨域问题是前端浏览器限制的，那让后端给每个响应加签名，前端验证签名，不就可以绕过跨域限制直接调后端了吗？

想法很好，但现实中有几个问题：

### 1. 签名不能解决跨域

浏览器的**同源策略**是浏览器层面的强制安全机制，**不看你有没有签名**。只要前端 JS 发起的请求目标域名、端口、协议和当前页面不一致，浏览器就直接拦截响应，根本不给你 JS 读到响应内容的机会。签名验证是在响应到达之后做的，但在那之前跨域检查已经把响应挡掉了。

### 2. 密钥怎么安全交给前端

服务端签名要求前端持有密钥来验证签名。但前端代码在浏览器里是完全暴露的，任何 JS 加密的密钥都能被用户通过 DevTools 提取出来。

| 方案 | 问题 |
|------|------|
| 密钥硬编码在前端代码里 | 打包后明文字符串搜一下就能找到 |
| 启动时从后端请求密钥 | 密钥本身也在网络上传输，和直接调 API 没区别 |
| 前端用公钥验证、后端用私钥签名 | 防不了重放攻击，且私钥分发到前端不再安全 |
...
- **服务端**：每个响应都要算一次 HMAC/RSA 签名
- **客户端**：每个响应都要验签，增加了 JS 执行时间
- **弱网环境**：加了签名后包体变大，传输更慢

相比之下 Nginx 的反向代理几乎零额外开销（就是转发 TCP 包）。

### 4. 破坏浏览器原生功能

- **无法利用浏览器缓存** — 签名为每个响应打上唯一标记，破坏缓存机制
- **打断浏览器下载行为** — 文件下载、断点续传等依赖标准 HTTP 协议，加签名会让这些功能复杂化
- **影响调试体验** — 浏览器 DevTools 的网络面板能看到原始请求和响应，加签名后查看明文内容还要额外解码

### 5. 谁在用服务端签名

服务端签名确实有实际应用的场景，但不是用来解决跨域的：

- **开放 API 鉴权** — 如 AWS Signature V4、阿里云 API 签名，给第三方开发者用的，不是浏览器端调用的
- **IoT/服务间通信** — 后端之间互相调用，没有浏览器跨域问题
- **支付回调验签** — 微信/支付宝的回调通知用签名验证来源，这是后端到后端的

### 6. 正确的做法

前后端分离的跨域问题，主流做法就三个：

1. **Nginx 反向代理**（推荐） — 同源访问，不涉及跨域，零开销
2. **后端 CORS 配置** — 后端允许指定域名跨域访问，但请求还是跨域的
3. **JSONP**（仅支持 GET，安全性差，已过时）

Nginx 反向代理是最简单也最广泛使用的方案，没有之一。
...
有人可能会想：既然只是加密传输，那让后端 Java（SpringBoot 内嵌 Tomcat）自己生成一个自签名证书不就行了？干嘛还要 Nginx 反代 + 申请 Let's Encrypt 这么麻烦？

自签证书确实能加密，但有以下几个问题：

### 1. 浏览器不信任，用户直接劝退

自签证书由你自己生成，没有经过受信任的 CA（证书颁发机构）签名。浏览器打开时直接弹出大红屏：

> **您的连接不是私密连接** — 攻击者可能会试图窃取您的信息

对普通用户来说，看到这个页面基本就直接关掉了。如果是公司内部使用的 OA 系统，你可以让每个员工手动导入证书信任，但这种操作对非技术用户来说门槛很高。

而 Let's Encrypt 的证书由浏览器内置信任的 CA 签发，打开就是绿色小锁，用户无感。这个区别就是用户能不能直接打开你的网站。

### 2. 证书管理成本高

| 对比维度 | 自签证书 | Let's Encrypt |
|---------|---------|--------------|
| 签发方式 | 自己生成，每台机器都要单独操作 | 一键申请，全自动 |
| 有效期 | 随意设，但没提醒 | 90 天，宝塔自动续签 |
| 吊销 | 没法吊销 | CA 支持吊销 |
| 多域名 | 每个域名要单独生成 | 支持 SAN 证书，一个证书覆盖多个域名 |
| 信任链 | 需要手动分发根证书 | 浏览器内置信任 |

如果哪天证书到期了忘记续，自签证书无声无息就失效了，你得等用户报错才发现。Let's Encrypt 有自动续签机制，宝塔面板会在到期前自动续上。

### 3. 后端直接处理 SSL 有性能损失

Java 的 SSL/TLS 实现（JSSE）性能远不如 Nginx + OpenSSL。Nginx 处理 SSL 握手用的是 OpenSSL 原生 C 库，性能是 Java 层的数倍。

在 Nginx 层终结 SSL 意味着：
- SSL 握手由 Nginx 处理，Java 后端只处理明文 HTTP 请求
- Java 应用不需要在代码里管证书，配置也简单
- 更换证书只需要 reload Nginx，不需要重启 Java 进程

### 4. 自签证书适合什么场景

自签证书也不是一无是处，特定场景下很合适：

1. **开发环境** — 本地 `localhost` 测试 HTTPS 功能，没必要申请正式证书
2. **内网服务间通信** — 微服务之间用自签证书互相认证，不走公网
3. **k8s 集群内部** — Istio 等 Service Mesh 自动签发和管理内部证书
4. **IoT 设备** — 设备与服务端双向认证，自签证书更可控

但对于**面向用户的前后端分离项目**，老老实实用 Let's Encrypt 或者购买商业证书就好。
